...
function BagContour:load_overrides()
	self._overrides = {}
...
function BagContour:save_overrides()
...
function BagContour:update_overrides()
	self:load_overrides()
	self:save_overrides()
end
...
function BagContour:renew_label_examples()
	local carry_by_msm = {}
	for carry_id, data in pairs(tweak_data.carry) do
...
function BagContour:set_contour_colors()
...
function BagContour:can_hide_body_bag_contour()
...
function BagContour:get_contour_color(id)
	if self.option_menu_is_focused and self._overrides[id] then
		local twdc = tweak_data.carry
		return self.move_speed_modifiers[twdc.types[twdc[id].type].move_speed_modifier]
	else
		return tweak_data.contour.interactable[id]
	end
end

function BagContour:set_bag_contour(unit, contour_id)
	...
	for _, m in ipairs(unit:get_objects_by_type(Idstring('material'))) do
		if m:variable_exists(ids_contour_color) then
			m:set_variable(ids_contour_color, self:get_contour_color(contour_id))
			m:set_variable(ids_contour_opacity, 1)
		end
	end
end

function BagContour:set_all_bags_contour()
	for i, carry_id in ipairs(self.examples) do
		self:set_bag_contour(self._bags[i], carry_id)
	end
	self:set_bag_contour(self._bodybag, 'person')
end

function BagContour:set_all_labels_color()
	for i, carry_id in ipairs(self.examples) do
		local c = self:get_contour_color(carry_id)
		self._ptexts[i]:set_color(Color(c.x, c.y, c.z))
	end
end

function BagContour:spawn_weight_bag(carry_id, h)
...
function BagContour:spawn_custom_bag()
	...
	if ids_custom_name and managers.dyn_resource:is_resource_ready(ids_unit, ids_custom_name, drp) then
		self:do_spawn_custom_bag(ids_custom_name)
	else
		self:do_spawn_custom_bag(ids_lootbag_name)
		-- Nope, following doesn't work
		-- self._custom_dyn_resource = ids_custom_name
		-- managers.dyn_resource:load(ids_unit, ids_custom_name, drp, callback(BagContour, BagContour, 'do_spawn_custom_bag', ids_custom_name))
	end
end

function BagContour:do_spawn_custom_bag(ids_unit_name)
	...
	self:set_bag_contour(self._custombag, self._customizing_carry_id)
end

function BagContour:spawn_body_bag()
...
function BagContour:update_colors()
	self:set_contour_colors()
	if not managers.hud then
		self:set_all_labels_color()
		self:set_all_bags_contour()
	end
end

function BagContour:create_weight_labels()
...
function BagContour:remove_weight_labels()
...
function BagContour:set_scene(resource)
	if resource then
		self[resource] = true
	end
	...
	if self.lootbag_is_ready and self.bodybag_is_ready then
		for i, carry_id in ipairs(self.examples) do
			self:spawn_weight_bag(carry_id, i)
		end
		self:spawn_body_bag()
		self:set_all_bags_contour()
		self:create_weight_labels()
		self:set_all_labels_color()
	end
end

function BagContour:init_weight_sliders()
...
Hooks:Add('MenuManagerInitialize', 'MenuManagerInitialize_BagContour', function(menu_manager)
	...
	local function _BagContourCustomize_SetHSV()
		...
		BagContour:set_bag_contour(BagContour._custombag, carry_id)
	end
	...
	MenuCallbackHandler.BagContourChangedFocus = function(node, focus)
		...
		local drp = managers.dyn_resource.DYN_RESOURCES_PACKAGE
		if focus then
			BagContour:init_weight_sliders()
			if not managers.network:session() then

				local lootbag_ready = managers.dyn_resource:is_resource_ready(ids_unit, ids_lootbag_name, drp)
				if not lootbag_ready then
					managers.dyn_resource:load(ids_unit, ids_lootbag_name, drp, callback(BagContour, BagContour, 'set_scene', 'lootbag_is_ready'))
				end

				local bodybag_ready = managers.dyn_resource:is_resource_ready(ids_unit, ids_bodybag_name, drp)
				if not bodybag_ready then
					managers.dyn_resource:load(ids_unit, ids_bodybag_name, drp, callback(BagContour, BagContour, 'set_scene', 'bodybag_is_ready'))
				end

				BagContour:set_scene()
			end
			...
			BagContour:remove_weight_labels()
