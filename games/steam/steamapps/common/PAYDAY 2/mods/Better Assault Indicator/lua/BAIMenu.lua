...
BAIMenu = class()
BAIMenu.make_fine_text = BlackMarketGui.make_fine_text
function BAIMenu:init()
    ...
    self:GetMenuFromJson(BAI.MenuPath .. "assault_box.json", BAI.settings.assault_panel.assault, BAI.settings.assault_panel.captain, BAI.settings.assault_panel.holdout)
    local assault_menu = self:GetMenu("bai_assault_box_menu")
    for _, item in pairs(assault_menu.items) do -- Hack, fixes 'mods/Better Assault Indicator/lua/BAIMenu.lua:1476: attempt to concatenate local 'text' (a table value)'
        if string.find(item.id, "captain_box_custom_text") then
            local length = item.callback == "SetCustomText" and "" or "short_"
            local v = BAI.settings.assault_panel.captain[length .. "custom_text"]
            item.default_value = v
            item.value = v
            item.panel:child("value"):set_text(v)
        end
    end
    ...
    self:GetMenuFromJson(BAI.MenuPath .. "hudlist.json", BAI.settings.hud.hudlist)
    --self:GetMenuFromJson(BAI.MenuPath .. "other.json")
...
function BAIMenu:GetMenuFromJson(path, ...)
...
function BAIMenu:CreateItem(item, items, menu_id, ...)
...
function BAIMenu:CreateOneLineItems(item, items, menu_id, ...)
    local offset = {
        label = 5,
        button = 10,
        toggle = 34,
...
function BAIMenu:AddItemToMenu(menu_id, item, total_size)
